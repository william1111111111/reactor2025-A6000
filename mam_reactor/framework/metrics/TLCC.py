...
def _func(pred_item, sp_item):
    # pred_item: Tensor([num_preds, l, 25])
    # sp_item: Tensor([l, 25])
    for i in range(pred_item.shape[0]):
        peak, center, offset = calculate_tlcc(
            pred_item[i].float(),
            sp_item.float(),
        )
...
def compute_TLCC(preds, speakers, p=8):
    # preds: List: [Tensor([num_preds, l, 25]), Tensor([num_preds, l', 25]), ...]
    # speakers: List: [Tensor([l, 25]), Tensor([l', 25]), ...]
    offset_list = []
    ...
    with mp.Pool(processes=p) as pool:
        # use map
        offset_list += pool.starmap(_func, zip(preds, speakers))
