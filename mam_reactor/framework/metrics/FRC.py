...
def _func(target, pred):
    ...
    for i in range(pred.shape[0]):
        ...
        for j in range(target.shape[0]):
            ccc, lst = concordance_correlation_coefficient(target[j].numpy(), pred[i].numpy())
            # if i == 0:
            #     print(f"25 dimension ccc lst: {lst} at j={j}")
            ccc_list.append(ccc)
        max_ccc_sum += max(ccc_list)
    return max_ccc_sum


def compute_FRC(preds, targets, p=1):
    FRC_list = []
    with mp.Pool(processes=p) as pool:
        FRC_list += pool.starmap(_func, zip(targets, preds))
