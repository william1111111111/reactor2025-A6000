...
from dataset.tools.util import resample_audio
...
class AudioProcessor:
    ...
    def __init__(
        ...
        self.wav2vec_feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained(wav2vec_model_path,
                                                                                  local_files_only=True)

    def preprocess(self, wav_file: str, clip_length: int = -1, seq_len: int = None):
        """
        ...
        if self.audio_separator is not None:
            ...
            vocal_audio_file = resample_audio(vocal_audio_file,
                                              os.path.join(self.audio_separator.output_dir,
                                                           f"{vocal_audio_name}-16k.wav"),
                                              self.sample_rate)
        else:
        ...
        audio_feature = np.squeeze(
            self.wav2vec_feature_extractor(speech_array, sampling_rate=sampling_rate).input_values)
        # compute sequence length based on video fps
        if seq_len is None:
            seq_len = math.ceil(len(audio_feature) / self.sample_rate * self.fps)
        else:
            if math.ceil(len(audio_feature) / self.sample_rate * self.fps) != seq_len:
                print(f"Warning: seq_len {seq_len} is not equal to "
                      f"computed version {math.ceil(len(audio_feature) / self.sample_rate * self.fps)}")
        audio_length = seq_len

        audio_feature = torch.from_numpy(audio_feature).float().to(device=self.device)

        if clip_length > 0 and seq_len % clip_length != 0:
            audio_feature = torch.nn.functional.pad(audio_feature, (0, (clip_length - seq_len % clip_length) * (
                        self.sample_rate // self.fps)), 'constant', 0.0)
